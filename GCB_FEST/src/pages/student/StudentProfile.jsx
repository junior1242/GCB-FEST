import { User, Mail, GraduationCap, Hash, BookOpen, Loader2 } from "lucide-react";
    ...
    <div className="text-white max-w-2xl mx-auto px-4 py-6 md:py-10">
      ...
      <div className="bg-white/5 border border-white/10 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 backdrop-blur-xl">
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-xl md:text-2xl font-bold italic">Student Account</h2>
        </div>

        <div className="space-y-4">
          {profileList.map((student, index) => (
            <div key={index} className="grid grid-cols-1 gap-4"> 
              <ProfileItem icon={<User size={18}/>} label="Full Name" value={student.name} />
              <ProfileItem icon={<Mail size={18}/>} label="Email" value={student.email} />
              <ProfileItem icon={<Hash size={18}/>} label="Roll Number" value={student.rollNumber} />
              <ProfileItem icon={<BookOpen size={18}/>} label="Department" value={student.department} />
              <ProfileItem icon={<GraduationCap size={18}/>} label="Semester" value={student.semester} />
            </div>            
          ))}
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-white/5 rounded-xl border border-white/5 hover:bg-white/10 transition-colors">
      ...
      <div className="min-w-0">
        ...
        <p className="text-slate-200 font-semibold truncate text-sm md:text-base">{value || "N/A"}</p>
      </div>
    </div>
  );
}